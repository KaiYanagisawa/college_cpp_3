...
int Person::ages(Date today)
{
    ...
    int number_of_year{today.get_year() - birth_day.get_year()};
    return (today.days_from_newyearsday() >= birth_day.days_from_newyearsday()) ? number_of_year : number_of_year - 1;
}

/*
今日が誕生日の場合：「今日が誕生日です。お誕生日おめでとう！」
誕生日後の場合：「今年の誕生日は過ぎてしまいました」
誕生日がまだ来ていない場合：「誕生日まであと??日です」
を出力する
*/
